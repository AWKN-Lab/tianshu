...
> **AWKN server override:** Code-quality commands run on the Windows workspace only. The server contains no `.git` file/directory and runs no Git command; remote health validation uses the deployed release ID, artifact SHA-256, process/API/metric evidence and the parent deployment policy.
