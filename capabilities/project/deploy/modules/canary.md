...
> **AWKN server override:** This module runs from the Windows orchestration workspace only. Never copy its repository, `.git`, or Git commands to the server. Remote canary checks consume release ID, artifact SHA-256, URLs and metrics only; the parent deployment policy takes precedence over all Git-related text below.
