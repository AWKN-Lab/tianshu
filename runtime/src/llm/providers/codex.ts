...
export class CodexProvider implements LlmProviderInterface {
  ...
  async chat(req: ChatRequest): Promise<ChatResponse> {
    ...
    if (!apiKey) throw new Error('AWKN_CODEX_API_KEY is not set');
    const model = req.model ?? process.env.AWKN_CODEX_MODEL ?? 'gpt-4o';
    const baseUrl = process.env.AWKN_CODEX_BASE_URL ?? 'https://api.openai.com/v1';
    ...
    try {
      ...
      if (controller.signal.aborted) throw new Error(`Codex API timeout after ${getLlmTimeoutMs()}ms`);
      throw new Error(`Codex API fetch failed: ${String(err)}`);
    } finally {
      clearTimeout(timeout);
    }

    if (!resp.ok) throw new Error(`Codex API ${resp.status}: ${await resp.text()}`);
    ...
    if (!choice) throw new Error('Codex API returned no choices');
    if (!data.usage) throw new Error('Codex API returned no usage field');
    if (!choice.message.content && !choice.message.tool_calls?.length) {
      throw new Error(`Codex API returned empty content with no tool_calls (finish_reason: ${choice.finish_reason})`);
    }
