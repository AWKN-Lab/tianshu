...
// ===========================================================================
// Section: Source Isolation (设计文档第 11 章 Registry 边界)
// ===========================================================================

/** 允许的 Policy 来源（与 Registry ALLOWED_SOURCES 一致） */
const ALLOWED_SOURCES = ['core', 'project', 'task_profile', 'evolve_candidate'] as const;

/**
 * 检查来源是否允许注册 (设计文档第 11 节).
 *
 * 允许: core / project / task_profile / evolve_candidate
 * 禁止: gundam / value / win / mr.mont / annie / subtitle 等业务项目
 */
...
/**
 * 获取来源的 Authority 等级 (设计文档第 6 节).
 *
 * core > task_profile > project > evolve_candidate
 * 高 Authority 的 Policy 在冲突解析中胜出.
 */
export function sourceAuthority(source: string): number {
  return AUTHORITY_LEVEL[source] ?? -1;
}

/**
 * 禁止注册的业务仓库 Policy 前缀 (设计文档第 11.2 节)
 */
...
/**
 * 检查 policyId 是否属于禁止的业务仓库 (设计文档第 11.2 节).
 */
export function isForbiddenPolicyId(policyId: string): boolean {
  const lowerId = policyId.toLowerCase();
  return FORBIDDEN_POLICY_ID_PREFIXES.some((prefix) => lowerId.startsWith(prefix));
}
